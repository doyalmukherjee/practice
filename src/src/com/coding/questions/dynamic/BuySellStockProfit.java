	...
	public int oneProfit(int arr[]) {
		...
		for (int i = 1; i < arr.length; i++) {
			if (arr[i] - minPrice > maxProfit) {
				maxProfit = arr[i] - minPrice; // reassign profit if element in array-minprice> previous profit value
			}
			if (arr[i] < minPrice) { // reassign profit to get the lowest value always
				minPrice = arr[i];
			}
		}
		return maxProfit;
